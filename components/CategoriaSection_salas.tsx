...
interface CategoriaSection_salasProps {
  titulo: string
  subtitulo: string
  salas: Salas[]
  verMasLink?: string
...
export function CategoriaSection({ titulo, subtitulo, salas, verMasLink }: CategoriaSection_salasProps) {
  // Si no hay equipos de la BD, mostrar placeholders
  const displaySalas = salas.length > 0 ? salas : getMockSalas()
    ...
    <section className="py-2 px-8 bg-white">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {displaySalas.slice(0, 3).map((salas, index) => (
            <ProductCard
              key={salas.ID_Sala || index}
              salas={salas}
              showBadge={index === 0}
              badgeText="DESTACADO"
            />
        ...
        {displaySalas.length > 3 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
            {displaySalas.slice(3, 6).map((salas, index) => (
              <ProductCard
                key={salas.ID_Producto || index + 3}
                salas={salas}
              />
            ))}
          </div>
